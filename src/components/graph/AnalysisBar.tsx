...
const num = "w-20 rounded bg-stone-800/80 px-1.5 py-0.5 text-right font-mono text-vermilion-100 tabular-nums outline-none focus:ring-1 focus:ring-vermilion-400";
        ...
        {TOOLS.map((t) => (
          <button
            ...
            className={`rounded px-2.5 py-1 transition ${
              tool === t.id ? "bg-vermilion-500/15 text-vermilion-200 ring-1 ring-vermilion-400/40" : "text-stone-400 hover:bg-white/5"
            }`}
          >
            {t.label}
          </button>
      ...
      {tool === "integral" ? (
        <>
          ...
          <span className="font-mono text-stone-300">
            ...
            <span className="text-vermilion-300">{readout?.kind === "integral" ? fmt(readout.value) : "—"}</span>
          </span>
        </>
      ) : (
        <>
          ...
          {readout?.kind === "point" && (
            <span className="flex flex-wrap items-center gap-3 font-mono text-stone-300">
              <span>f(x) = <span className="text-vermilion-300">{fmt(readout.fa)}</span></span>
              <span>f'(x) = <span className="text-emerald-300">{fmt(readout.slope)}</span></span>
              {tool === "derivative" && (
                <span className="text-stone-500">f'(x) = {readout.dExpr}</span>
              )}
            </span>
          )}
        </>
