...
export const EXAMPLES: Example[] = [
  ...
  // ─── Phase IV — Linear Algebra ────────────────────────────────────────────────────
  {
    ...
    build: () => make("2D Linear Transformation", "Visualize a 2×2 matrix as a linear map on the unit square/circle.",
      [
        { id: "m0", kind: "markdown", source: "# 2D Linear Transformation\n\nEvery 2×2 matrix $A = \\begin{pmatrix}a&b\\\\c&d\\end{pmatrix}$ acts on $\\mathbb{R}^2$ by $x \\mapsto Ax$. The Inspector reveals eigenstructure, SVD, and geometric action." },
        { id: "e0", kind: "expression", name: "A", source: "[[2, 1], [1, 2]]" },
        { id: "a0", kind: "analysis", targetName: "A" },
      ]),
  },
  {
    id: "lin-svd",
    ...
    build: () => make("SVD of a 3×2 matrix", "U·Σ·Vᵀ decomposition with condition number.",
      [
        { id: "m0", kind: "markdown", source: "# Singular Value Decomposition\n\nFor any $m\\times n$ matrix $A$, the SVD gives $A = U\\Sigma V^{\\mathsf{T}}$ with orthogonal $U,V$ and diagonal $\\Sigma \\ge 0$. The singular values reveal rank and conditioning." },
        { id: "e0", kind: "expression", name: "B", source: "[[1, 2], [3, 4], [5, 6]]" },
        { id: "a0", kind: "analysis", targetName: "B" },
      ]),
  },
  {
    ...
    build: () => make("Least Squares", "Overdetermined system → QR + back-substitution gives the minimum-norm solution.",
      [
        { id: "m0", kind: "markdown", source: "# Least Squares via QR\n\nGiven $A\\in\\mathbb{R}^{m\\times n}$ (with $m\\ge n$) and $b\\in\\mathbb{R}^m$, the problem $\\min_x \\|Ax-b\\|_2$ is solved by the normal equations $A^{\\mathsf{T}}Ax = A^{\\mathsf{T}}b$ or, more stably, by QR: $A = QR \\Rightarrow Rx = Q^{\\mathsf{T}}b$." },
        { id: "e0", kind: "expression", name: "A", source: "[[1, 2], [3, 4], [5, 6], [7, 8]]" },
        { id: "e1", kind: "expression", name: "b", source: "[1, 2, 3, 4]" },
        { id: "a0", kind: "analysis", targetName: "A" },
      ]),
  },
  ...
  // ─── Phase IV — Dynamical Systems ─────────────────────────────────────────────────
  {
    ...
    build: () => make("Harmonic Oscillator", "Continuous 2D flow with a center equilibrium at the origin.",
      [
        { id: "m0", kind: "markdown", source: "# Harmonic Oscillator\n\n$$\n\\begin{cases}\n\\dot{x} = y \\\\\n\\dot{y} = -x\n\\end{cases}\n$$\nEquilibrium at $(0,0)$. Jacobian $\\begin{pmatrix}0&1\\\\-1&0\\end{pmatrix}$ has eigenvalues $\\pm i$ ⇒ linear center. Nonlinear terms vanish, so it's a true center with closed orbits." },
        { id: "e0", kind: "expression", name: "sys", source: "[y, -x]" },
        { id: "a0", kind: "analysis", targetName: "sys" },
      ]),
  },
  {
    id: "dyn-lorenz",
    ...
    build: () => make("Lorenz Attractor", "3D chaotic flow with a strange attractor and sensitive dependence.",
      [
        { id: "m0", kind: "markdown", source: "# Lorenz System\n\n$$\n\\begin{cases}\n\\dot{x} = \\sigma(y-x) \\\\\n\\dot{y} = x(\\rho-z)-y \\\\\n\\dot{z} = xy - \\beta z\n\\end{cases}\n$$\nClassic parameters: $\\sigma=10, \\rho=28, \\beta=8/3$. Three equilibria: origin (saddle) and two symmetric ones (unstable spirals)." },
        ...
        { id: "e0", kind: "expression", name: "lorenz", source: "[sigma*(y-x), x*(rho-z)-y, x*y-beta*z]" },
        { id: "a0", kind: "analysis", targetName: "lorenz" },
      ]),
  },
  // ─── Phase IV — ODEs ──────────────────────────────────────────────────────────────
  {
  ...
  },
  {
    ...
    build: () => make("Harmonic Oscillator ODE", "Compare fixed-step Euler and RK4 on energy conservation.",
      [
        ...
        { id: "e0", kind: "expression", name: "osc", source: "[v, -x]" },
        { id: "a0", kind: "analysis", targetName: "osc" },
      ]),
  },
  // ─── Phase IV — PDEs ──────────────────────────────────────────────────────────────
  {
  ...
  },
  {
    ...
    build: () => make("Linear Regression", "Dataset of points → linear and quadratic least-squares.",
      [
        { id: "m0", kind: "markdown", source: "# Linear Regression\n\nGiven data $\\{(x_i, y_i)\\}$, find $a,b$ minimizing $\\sum (y_i - a - b x_i)^2$. The normal equations solve this exactly." },
        { id: "e0", kind: "expression", name: "data", source: "[[0,1], [1,3], [2,2.5], [3,5], [4,4.8]]" },
        { id: "a0", kind: "analysis", targetName: "data" },
      ]),
  },
  // ─── Phase IV — Number Theory ─────────────────────────────────────────────────────
  {
    ...
    build: () => make("Collatz Exploration", "Iterate n → n/2 (even) or 3n+1 (odd) from many seeds.",
      [
        { id: "m0", kind: "markdown", source: "# Collatz Conjecture\n\n$$T(n) = \\begin{cases}n/2 & n \\text{ even} \\\\ 3n+1 & n \\text{ odd}\\end{cases}$$\nThe conjecture: every positive integer eventually reaches the cycle 4→2→1. Plot stopping times vs seed." },
        { id: "e0", kind: "expression", name: "collatz", source: "(n % 2 == 0) ? n/2 : 3*n+1" },
